...
3. Our system ofen use `llama3-8b`/`qwen3-8b` so sometime encounter `No tool calls generated` in `Query generator agent` especially `llama3-8b`, within the `query_generator_node` function, code can be rewritten to force a database lookup for each query generated.
