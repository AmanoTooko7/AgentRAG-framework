...
1. RAG Database are Lilian Weng's blog and 2018 wikipedia dump, (but wiki dump's embedding database、index and .db files are too large up to 100G+ so i haven't upload), you can use weng's blog as databse through comment `from src.retriever_of_wiki import build_retriever_tool` use `from src.retriever import build_retriever_tool` have a try.
